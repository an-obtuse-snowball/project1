...
function loadNewsData(isoCode) {
    var newsElement ;
    $.ajax({
        url: "./libs/php/loadNews.php",
        type: 'get',
        dataType: 'json',
        data: {
            iso: isoCode
        ...
        success: function (response) {
            ...
            $.each(response.data.articles, function (i, value) {
                $('#thumbnail'+i).attr('src', response.data.articles[i].urlToImage);
                $('#newsTitle'+i).html("<a href="+response.data.articles[i].url+">"+response.data.articles[i].title+"</a>");
                $('newsContent'+i).html(response.data.articles[i].title);
            })
        },
...
$(document).ready(function () {
    ...
    //Locks to Current Country
    L.easyButton('bi bi-geo-fill', function () {
        getIsoFromCoords(user.latitude, user.longitude);
    }, 'Return to your current location').addTo(worldMap);

    //Shows news of currently loaded currently
    L.easyButton('bi bi-newspaper', function () {

        $('#ModalNews').show();

    }, 'See the latest news in this country').addTo(worldMap);

    //Shows covid data
    L.easyButton('bi bi-book', function () {

    }, 'Display Wiki content about this country').addTo(worldMap);
});
